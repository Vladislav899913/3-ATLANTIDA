...
const navLinks = document.querySelector('#nav_links').cloneNode(1);
...
function showSlides(n) {
    let i;
    let slides = document.getElementsByClassName('prices_slider_item');
    let dots = document.getElementsByClassName('prices_dots_dot');
    ...
    for (i = 0; i < dots.length; i++) {
        dots[i].className = dots[i].className.replace(' prices_dot_active', '');
    }

    slides[slideIndex - 1].style.display = 'block';
    dots[slideIndex - 1].className += ' prices_dot_active';
}
